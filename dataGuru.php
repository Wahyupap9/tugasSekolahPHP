...
<html lang="en">
  ...
  <body>
    <h1 class="title-data">Data Guru</h1>
    <!-- search -->
    ...
    <?php if(isset($_GET['id']) && isset($_GET['page']) == $data) : 
        $id = $_GET['id'];
        $queryPopup = "SELECT * FROM data_guru WHERE id = $id";
        $showDetails = query($queryPopup)[0]; ?>
        <div class="overlay dNone">
         <div class="popup">
          <div class="close-button"> <a href="?page=<?=$data;?>"><i class="ri-close-large-line"></i></a></div>
    ...
    <div class="container-data">
      <?php foreach ($box as $item) : ?>
        <div class="item <?php if($item['jenis_kelamin'] === "Perempuan") {
          ...
          <div class="buttons">
            <a href="editGuru.php?id=<?= $item['id']; ?>"><i class="ri-edit-2-fill"></i></a>
            <a href="delete.php?id=<?= $item['id']; ?>&data=data_guru"><i class="ri-delete-bin-fill"></i></a>
          </div>
          ...
          <a href="?id=<?= $item['id']; ?>&page=<?=$data;?>"><button class="showDetails">Detail</button></a>
        </div>
